...
class VoxelRCNNBasedHead(RoIHeadTemplate):
    def __init__(self, backbone_channels, model_cfg, point_cloud_range, voxel_size, num_class=1, **kwargs):
        ...
        # self.cls_pred_up_layer = nn.Linear(pre_channel, self.num_class, bias=True)
        # self.cls_pred_mid_layer = nn.Linear(pre_channel, self.num_class, bias=True)
        # self.cls_pred_down_layer = nn.Linear(pre_channel, self.num_class, bias=True)
        ...
        # self.reg_pred_up_layer = nn.Linear(pre_channel, self.box_coder.code_size * self.num_class, bias=True)
        # self.reg_pred_mid_layer = nn.Linear(pre_channel, self.box_coder.code_size * self.num_class, bias=True)
        # self.reg_pred_down_layer = nn.Linear(pre_channel, self.box_coder.code_size * self.num_class, bias=True)
    ...
    def init_weights(self):
        ...
        # nn.init.normal_(self.cls_pred_up_layer.weight, 0, 0.01)
        # nn.init.constant_(self.cls_pred_up_layer.bias, 0)
        # nn.init.normal_(self.reg_pred_up_layer.weight, mean=0, std=0.001)
        # nn.init.constant_(self.reg_pred_up_layer.bias, 0)
        #
        # nn.init.normal_(self.cls_pred_mid_layer.weight, 0, 0.01)
        # nn.init.constant_(self.cls_pred_mid_layer.bias, 0)
        # nn.init.normal_(self.reg_pred_mid_layer.weight, mean=0, std=0.001)
        # nn.init.constant_(self.reg_pred_mid_layer.bias, 0)
        #
        # nn.init.normal_(self.cls_pred_down_layer.weight, 0, 0.01)
        # nn.init.constant_(self.cls_pred_down_layer.bias, 0)
        # nn.init.normal_(self.reg_pred_down_layer.weight, mean=0, std=0.001)
        # nn.init.constant_(self.reg_pred_down_layer.bias, 0)
    ...
    def forward(self, batch_dict):
        ...
        # up_features = shared_features[up_mask]
        # mid_features = shared_features[mid_mask]
        # down_features = shared_features[down_mask]
        #
        # batch_size_rcnn = shared_features.shape[0]
        # rcnn_cls = shared_features.new_zeros((batch_size_rcnn, 1))
        # rcnn_reg = shared_features.new_zeros((batch_size_rcnn, 7))
        #
        # rcnn_cls_up = self.cls_pred_up_layer(up_features)
        # rcnn_cls_mid = self.cls_pred_mid_layer(mid_features)
        # rcnn_cls_down = self.cls_pred_down_layer(down_features)
        #
        # rcnn_reg_up = self.reg_pred_up_layer(up_features)
        # rcnn_reg_mid = self.reg_pred_mid_layer(mid_features)
        # rcnn_reg_down = self.reg_pred_down_layer(down_features)
        #
